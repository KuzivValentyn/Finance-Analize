...
try:
    # Read CSV with date parsing and handle potential errors
    df = pd.read_csv(
        data_file,
    ...
    # Remove rows where index couldn't be parsed as date (e.g., "Ticker" headers)
    df = df[pd.to_datetime(df.index, errors='coerce').notna()]
    
    # Set frequency only if index is DatetimeIndex
    if isinstance(df.index, pd.DatetimeIndex):
        df = df.asfreq('D')  # Set daily frequency (adjust as needed: 'M' for monthly, 'Q' for quarterly, etc.)
    
    # Convert data to numeric types and drop rows with missing values
